...
export const options = {
  ...
  thresholds: {
    ...
    'http_req_duration{name:POST /queue/enter/:eventId}': ['p(95)<500', 'p(99)<1000'],
    'http_req_duration{name:GET /queue/status/:eventId}': ['p(95)<200', 'p(99)<500'],
...
export default function (data) {
  ...
  group('browse', () => {
    const eventsRes = http.get(`${BASE_URL}/events`, { ...authHeaders, tags: { name: 'GET /events' } });
  ...
  if (targetEventId) {
    group('queue', () => {
      const enterRes = http.post(
        ...
        null, 
        { ...authHeaders, tags: { name: 'POST /queue/enter/:eventId' } }
      );
      ...
      while (retries > 0) {
        const statusRes = http.get(
          ...
          { ...authHeaders, tags: { name: 'GET /queue/status/:eventId' } }
        );
  ...
  if (targetEventId && isQueuePassed) {
    group('reservation', () => {
      const seatsRes = http.get(`${BASE_URL}/seats/event/${targetEventId}`, { ...authHeaders, tags: { name: 'GET /seats/event/:eventId' } });
